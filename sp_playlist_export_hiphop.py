...
for l_Playlist in l_PlaylistList:
    ...
    l_DirPath = os.path.join('/run/media/benweet/2B29-1EE6', l_Playlist)
    if not os.path.isdir(l_DirPath):
        os.mkdir(l_DirPath)
        
    while(True):
        l_Result = subprocess.call(['./sp_playlist_export', 'Benweet', l_Args.passwd, '/run/media/benweet/2B29-1EE6', l_Playlist])
        if l_Result != 1:
            break
